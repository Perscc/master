...
class Operation:
    ...
    # move_mouse 移动鼠标
    def move_mouse(self, img, fingers, action, cLocx, cLocy):
    ...
    # left_click 鼠标左键
    def left_click(self, img, fingers, action, distance, x, y):
        """
        img: 摄像头获取的当前帧
        fingers： 当前帧手指状态
        distance：食指与中指的距离
    ...
    def press_down(self, img, fingers, action, active_window_process_name):
    ...
    def control_colume_level(self, img, fingers, action, cLocx, cLocy, volume):
    ...
    def detect(self, detector):
        ...
        while True:
            ...
            # 显示图像，输入窗口名及图像数据
            self.info_print(img)
            if cv2.waitKey(1) & 0xFF == 27 or cv2.getWindowProperty('frame',
                                                                    cv2.WND_PROP_VISIBLE) < 1.0:  # 每帧滞留20毫秒后消失，ESC键退出或鼠标关闭窗口
                break
